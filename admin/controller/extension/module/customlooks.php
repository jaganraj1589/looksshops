<?php
class ControllerExtensionModuleCustomlooks extends Controller
{
  private $error = array();
  ...
  {
    $this->load->language('extension/module/customlooks');
    $this->document->setTitle($this->language->get('heading_title'));
    $this->load->model('setting/module');
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
      if (!isset($this->request->get['module_id'])) {
        $this->model_setting_module->addModule('customlooks', $this->request->post);
      } else {
    ...
    if (!isset($this->request->get['module_id'])) {
      $data['breadcrumbs'][] = array(
        ...
        'href' => $this->url->link('extension/module/customlooks', 'user_token=' . $this->session->data['user_token'], true)
      );
    } else {
      $data['breadcrumbs'][] = array(
        'text' => $this->language->get('heading_title'),
        'href' => $this->url->link('extension/module/customlooks', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
      );
    }
    if (!isset($this->request->get['module_id'])) {
      $data['action'] = $this->url->link('extension/module/customlooks', 'user_token=' . $this->session->data['user_token'], true);
    } else {
      $data['action'] = $this->url->link('extension/module/customlooks', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
    }
  ...
  {
    if (!$this->user->hasPermission('modify', 'extension/module/customlooks')) {
  ...
  public function install()
  {
    ...
    $this->model_setting_setting->editSetting('module_customlooks', ['module_customlooks_status' => 1]);
  }

  public function uninstall()
  {
    $this->load->model('setting/setting');
    $this->model_setting_setting->deleteSetting('module_customlooks');
  }
}
